            ...
            <form method="post" action="<?php echo U('Home/CreateBook/createBooktext');?>" name="textBookForm" enctype="multipart/form-data">
                ...
                <div class="lesson-img-box">
                    <ul class="weui-uploader__files" id="uploaderFiles">
                        ...
                        <li class="weui-uploader__file" style="background-image:url(/mobile/images/activity/activity_fengmian.png)" id="text_cover_img">
                        </li>
                    </ul>
                    <div class="weui-uploader__input-box">
...
<!--                     <p class="unit-aside">
 ...
 -->                </div>
                ...
                <input type="hidden" name="text_book_id">
            </form>
        </div>
        <!-- 图书列表-->
...
<script>
    $(function(){
                ...
                $.ajax({
                    ...
                    success:function(data){
                        ...
                        $.each(data,function(no,items){
                            ...
                            switch(col){
                                case 1: 
                                    var html = '<div class="activity-list-' + (index % 2 == 0 ? "left" : "right") + ' activity-list-common"><div class="img-box"><img src="' + items.cover_img + '" alt=""><p>' + (items.name) + '<span class="' + items.s_html + '">[' + items.status + ']</span></p></div><div class="function-list"><div class="list-box"><div class="fuction change_btn" data="' + items.id+ '">修改</div><div class="fuction del_btn" data="' + items.id+ '">删除</div></div></div></div>';
                                    break;
                                case 3:
                                    var html = '<div class="activity-list-' + (index % 2 == 0 ? "left" : "right") + ' activity-list-common"><div class="img-box"><img src="' + items.image + '" alt=""><p>' + (items.book_name).substr(0,5) +'<span style="font-weight: 200;margin-left:5px ">' + (items.name).substr(0,5) + '</span></p></div><div class="function-list"><div class="list-box"><div class="fuction change_btn" data="' + items.id + '">修改</div><div class="fuction del_text" data="' + items.id+ '">删除</div></div></div></div>';
                                    break;
                            }
                            res += html;
                            if (index % 2 != 0) {
        ...
        //修改课本
        $(".activity-list-book-all").on("click",".change_btn",function(){
            var id = $(this).attr('data');
            $.ajax({
                type: 'POST',
                url: "<?php echo U('Home/CreateBook/getBook');?>",
                data: {'id':id},
                success:function(res){
                    $($(".body-box-common")[0]).css("display","block").siblings().css("display","none");
        ...
        //删除课文
        $(".activity-list-all").on("click",".del_text",function(){
            var id = $(this).attr('data');
            $.ajax({
                type: 'POST',
                url: "<?php echo U('Home/CreateBook/deleteTextBook');?>",
                data: {'text_id':id},
                success:function(res){
                    weui.alert(res.message,function(){
                        window.location.reload();
        ...
        //修改课文
        $(".activity-list-all").on("click",".change_btn",function(){
            var id = $(this).attr('data');
            $.ajax({
                type: 'POST',
                url: "<?php echo U('Home/CreateBook/getTextBook');?>",
                data: {'id':id},
                success:function(res){
                    res = JSON.parse(res);
                    console.log(res);
                    $($(".body-box-common")[2]).css("display","block").siblings().css("display","none");
                    $("input[name='text_type']").val("change");
                    $("input[name='text_book_id']").val(res['id']);
                    $("input[name='bookName']").val(res['name']);
                    $('#text_cover_img').css("background-image","url(" + "<?php echo imageDomain('/');?>" + res['image'] + ")");
                }
            });
        });
